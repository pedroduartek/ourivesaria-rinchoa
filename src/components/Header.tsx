...
const linkBaseClass =
  'rounded-full px-4 py-2 text-sm font-semibold transition hover:bg-white/70 hover:text-forest'
    ...
    <header className="section-shell">
      <div className="panel sticky top-4 z-40 mt-4 px-4 py-3 sm:px-6">
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            type="button"
            className="inline-flex items-center justify-center rounded-full border border-bronze/10 bg-white/70 px-4 py-2 text-sm font-semibold text-forest shadow-sm lg:hidden"
        ...
        {isMenuOpen ? (
          <nav
            ...
            className="mt-4 grid gap-2 rounded-[1.5rem] border border-white/70 bg-white/75 p-3 lg:hidden"
          >
            {siteContent.navigation.map((item) => (
              <NavLink
                key={item.path}
